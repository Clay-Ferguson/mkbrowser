...
In this demo we will explore the advanced search feature by running a boolean query across the Federalist Papers content.
You can see the folder listing in front of us, including the federalist-papers folder.`
    ...
    writeNarration(
      screenshotDir,
      step++,
      `We are now inside the federalist-papers folder and can see all of its documents.
Now let's open the search menu to create a new advanced search.`
    );
...
By default it may be in a simple search mode.
We need to switch to Advanced mode so that we can use boolean expressions in our query.`
    ...
    writeNarration(
      screenshotDir,
      step++,
      `Advanced search mode is now active.
We can enter a boolean expression that will be evaluated against each file in the folder.`
    );
    ...
    writeNarration(
      screenshotDir,
      step++,
      `Here is the search query field.
We will enter a boolean "and" expression that finds only the files containing both the word "political" and the phrase "free people" at the same time.`
    ...
    writeNarration(
      screenshotDir,
      step++,
      `We have typed the expression dollar-sign "political" and-and dollar-sign "free people".
The double-ampersand operator means that both terms must appear in the same file for it to be included in the results.`
    );
    ...
    writeNarration(
      screenshotDir,
      step++,
      `We can also give this search a descriptive name so that it is saved and we can run it again later.
...
To access the search menu again, we first need to switch back to the Browse tab.`
    ...
    writeNarration(
      screenshotDir,
      step++,
      `We are back in the browser view.
The search icon is now visible at the top of the window.`
    );
    ...
    writeNarration(
      screenshotDir,
      step++,
      `The search menu now shows our saved "Political Free People Search" definition.
Any time we want to run that same boolean query again, we can simply click its name in this menu and MkBrowser will re-execute it instantly.
That is the power of the advanced search feature in MkBrowser.`
    );
