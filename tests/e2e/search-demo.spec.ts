...
test.describe('Search Demo', () => {
  test('search within federalist papers', async ({ mainWindow }) => {
    ...
    writeNarration(screenshotDir, step++, 'Welcome to MkBrowser. Today we\'ll be exploring the search feature by searching inside the Federalist Papers content. You can see the folder listing in front of us, including the federalist-papers folder.');
    ...
    writeNarration(screenshotDir, step++, 'Let\'s open the federalist-papers folder by clicking on it, so we can search within its contents.');
    ...
    writeNarration(screenshotDir, step++, 'We\'re now inside the federalist-papers folder. We can see the documents listed here. Now let\'s use the search feature to find specific content across these files.');
    ...
    writeNarration(screenshotDir, step++, 'The search menu has appeared. We\'ll click "New Search..." to open the search dialog and define a new query.');
    ...
    writeNarration(screenshotDir, step++, 'The search dialog is now open. We can enter our search term here. Let\'s search for the word "political" to find references to that topic across the Federalist Papers.');
    ...
    writeNarration(screenshotDir, step++, 'We\'ve typed "political" into the search field. MkBrowser will search through the content of all files in the current folder. Now let\'s run the search.');
    ...
    writeNarration(screenshotDir, step++, 'We\'ll click the Search button to execute the query and find all matching files.');
    ...
    writeNarration(screenshotDir, step++, 'The search results are now displayed. MkBrowser has found all the files containing the word "political" across the Federalist Papers. Each result shows the file path and matching context. Let\'s click on one of the results to jump directly to that file.');
    ...
    writeNarration(screenshotDir, step++, 'Let\'s click on the result for federalist-03-08.md to navigate directly to that file in the browser.');

    await demonstrateClickForDemo(targetResult);

    await mainWindow.waitForTimeout(500);
    await takeStepScreenshot(mainWindow, screenshotDir, step++, 'file-navigated');
    writeNarration(screenshotDir, step++, 'MkBrowser has jumped directly to the federalist-03-08.md file. The file is now visible in the browser, highlighted so you can see exactly where it appears in its folder. This is how easy it is to search and navigate to specific content in MkBrowser.');
