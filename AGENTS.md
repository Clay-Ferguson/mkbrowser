...
**Components must be module-level.** The compiler compiles only top-level components/hooks, so a component returned from a factory (`function make(x) { return function Foo() {…} }`) or declared inside another component can be silently skipped — not a bailout, just never attempted, so no bailout gate sees it (and the `react-hooks/*` lint rules that rely on component detection don't analyze it either). Never build components in factories or inside other functions; make them top-level and supply the closed-over values as props or via context (see `BlockClickContext` in `blockClickComponents.tsx` and `MarkdownEntryContext` in `markdownEntryContext.ts`, which feed the react-markdown `components` map as a module-level constant). `compiler-coverage.mjs` fails with `UNCOMPILED` on any PascalCase function containing JSX that the compiler neither compiled nor bailed on, so this breaks the package build. Likewise, a `use*` function is only treated as a hook if it actually calls a hook — one that calls none (e.g. `function useX(p) { return () => f(p); }`) is never compiled and hands callers a fresh value every render; `compiler-coverage.mjs` fails such functions with `UNCOMPILED HOOK`. Inline them at the call sites (where the compiler memoizes them) or rename them to a plain function.

**`memo()` is the exception.** The compiler already skips re-rendering children whose props are unchanged, except for rows created inside a `.map()` callback. So `memo()` is allowed only on per-row components in large `.map()` lists, or on components with an expensive render body (e.g. `MarkdownView`), and only when every prop is stable. Fix prop stability first, and give every `memo()` a comment explaining why its props are stable. Full rule: `docs/technical_notes/DEVELOPER_GUIDE.md` § "When `memo()` is (and isn't) warranted".

Three guards enforce this: the `react-hooks/todo` + `react-hooks/syntax` ESLint rules (errors in `eslint.config.mjs`); **`compiler-coverage.mjs`** at the repo root — the source of truth, since it runs the exact compiler version the build uses; and **`bundle-fingerprint.mjs`**, which runs *after* packaging and fails the build if the built renderer bundle contains less React Compiler output (`memo_cache_sentinel` occurrences) than the number of functions `compiler-coverage.mjs` compiled — the only check that can catch the compiler being silently configured out of the Vite pipeline (e.g. a broken `vite.renderer.config.mts` wiring). Both scripts run as **Forge hooks** in `forge.config.ts` (`prePackage` runs compiler-coverage, `postPackage` runs bundle-fingerprint), so every `npm run package` / `npm run make` — and therefore `build.sh`, `playwright-test.sh`, and the e2e auto-build — is gated with no way to skip them; `pre-package.sh` keeps only the tests + lint gate. `node compiler-coverage.mjs` scans all of `src/` and exits 1 on any bailout; `node compiler-coverage.mjs <file>` gives a verbose per-function report. After touching components/hooks, confirm the file still reports all `OK`. Full details, fix patterns, and the exhaustive-deps escape patterns: `docs/technical_notes/DEVELOPER_GUIDE.md` § React Compiler.
