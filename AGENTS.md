...
Two guards enforce this: the `react-hooks/todo` + `react-hooks/syntax` ESLint rules (errors in `eslint.config.mjs`), and **`compiler-coverage.mjs`** at the repo root — the source of truth, since it runs the exact compiler version the build uses. `node compiler-coverage.mjs` scans all of `src/` and exits 1 on any bailout (`pre-package.sh` runs it as a build gate — shared by `build.sh` and `playwright-test.sh`); `node compiler-coverage.mjs <file>` gives a verbose per-function report. After touching components/hooks, confirm the file still reports all `OK`. Full details, fix patterns, and the exhaustive-deps escape patterns: `docs/technical_notes/DEVELOPER_GUIDE.md` § React Compiler.
