...
# Answering 'y' automatically runs ../kocreator/create-video.sh
```
...
**Location**: `../kocreator/create-video.sh`
...
The `KOKORO_PROJECT_DIR` variable at the top of `../kocreator/create-video.sh` controls the path to the Kokoro project directory (default: `../kocreator`). Override it if your Kokoro project is in a different location.
...
6. **Generate video** (prompted automatically):
   ...
   ```bash
   ../kocreator/create-video.sh "$PWD" feature-name-demo
   ```
...
Edit `../kocreator/create-video.sh`:
```bash
...
Edit the FFmpeg command in `../kocreator/create-video.sh`:
```bash
...
├── create-video.sh                     # FFmpeg conversion script
...
- **Solution**: Delete the `generated-wav/` folder to force regeneration:
  ...
  ../kocreator/create-video.sh "$PWD" <test-name>
  ```
...
3. **Video generation**: `../kocreator/create-video.sh` converts text to speech via Kokoro TTS and produces final videos
