...
- **Settings:** the Settings view mutates the store live (`setFontSize`, `setSortOrder`, `toggleBookmark`, …) for instant UI feedback, and persistence happens by calling `saveSettings()` from `src/renderer/config.ts` (which writes `getSettings()` via `api.updateConfig` and reports a failure through `setAppError`). Nothing threads a save-settings callback through props — every caller imports it.
