...
[`create-video.sh`](create-video.sh) converts test output to videos:
...
# Answering 'y' automatically runs create-video.sh
```
...
The `KOKORO_PROJECT_DIR` variable at the top of `create-video.sh` controls the path to the Kokoro project directory (default: `../kocreator`). Override it if your Kokoro project is in a different location.
...
6. **Generate video** (prompted automatically):
   ...
   ```bash
   ./create-video.sh feature-name-demo
   ```
...
Edit the FFmpeg command in `create-video.sh`:
```bash
...
3. **Video generation**: `create-video.sh` converts text to speech via Kokoro TTS and produces final videos
