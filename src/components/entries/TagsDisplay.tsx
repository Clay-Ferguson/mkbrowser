 ...
 */
export default function TagsDisplay({ tags, floatRight = false }: TagsDisplayProps) {
  if (tags.length === 0) return null;

  if (floatRight) {
    return (
      <div
        className="flex flex-nowrap gap-x-2 ml-4 mb-2"
        style={{ fontFamily: MONO_FONT, float: 'right' }}
      >
        {tags.map((tag) => (
          <span
            key={tag}
